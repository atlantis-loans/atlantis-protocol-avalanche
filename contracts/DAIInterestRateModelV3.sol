...
contract DAIInterestRateModelV3 is JumpRateModelV2 {
    ...
    constructor(uint jumpMultiplierPerYear, uint kink_, address pot_, address jug_, address owner_) JumpRateModelV2(0, 0, jumpMultiplierPerYear, kink_, owner_) public {
        gapPerTimestamp = 4e16 / blocksPerYear;
        pot = PotLike(pot_);
        jug = JugLike(jug_);
        poke();
    }
    ...
    function updateJumpRateModel(uint baseRatePerYear, uint gapPerYear, uint jumpMultiplierPerYear, uint kink_) external {
        ...
        gapPerTimestamp = gapPerYear / blocksPerYear;
    ...
    function getSupplyRate(uint cash, uint borrows, uint reserves, uint reserveFactorMantissa) public view returns (uint) {
        ...
        if (underlying == 0) {
            ...
            uint cashRate = cash.mul(dsrPerTimestamp()).div(underlying);
     ...
     */
    function dsrPerTimestamp() public view returns (uint) {
        return pot
    ...
    function poke() public {
        (uint duty, ) = jug.ilks("AVAX-A");
        uint stabilityFeePerTimestamp = duty.add(jug.base()).sub(1e27).mul(1e18).div(1e27).mul(3);

        // We ensure the minimum borrow rate >= DSR / (1 - reserve factor)
        baseRatePerBlock = dsrPerTimestamp().mul(1e18).div(assumedOneMinusReserveFactorMantissa);

        // The roof borrow rate is max(base rate, stability fee) + gap, from which we derive the slope
        if (baseRatePerBlock < stabilityFeePerTimestamp) {
            multiplierPerBlock = stabilityFeePerTimestamp.sub(baseRatePerBlock).add(gapPerTimestamp).mul(1e18).div(kink);
        } else {
            multiplierPerBlock = gapPerTimestamp.mul(1e18).div(kink);
        }
