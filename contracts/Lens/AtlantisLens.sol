...
contract AtlantisLens {
    ...
    function getAtlantisBalanceMetadata(address atlantis, address account) external view returns (AtlantisBalanceMetadata memory) {
        return AtlantisBalanceMetadata({
            balance: IBEP20(atlantis).balanceOf(account),
            votes: 0, // (deprecated)
            delegate: account // (deprecated)
        });
    }

    struct AtlantisBalanceMetadataExt {
        uint balance;
        uint votes; // (deprecated)
        address delegate; // (deprecated)
        uint allocated;
    }

    function getAtlantisBalanceMetadataExt(address atlantis, ComptrollerLensInterface comptroller, address account) external returns (AtlantisBalanceMetadataExt memory) {
        uint balance = IBEP20(atlantis).balanceOf(account);
        comptroller.claimAtlantis(account);
        uint newBalance = IBEP20(atlantis).balanceOf(account);
